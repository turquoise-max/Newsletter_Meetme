...
### D. 프리미엄 에디터 및 인터랙티브 UI (Advanced UX)
*   **소스별 이미지 라이브러리**: 수집된 이미지를 출처(Article)별로 그룹화하여 보여주는 사이드바를 통해 사용자가 맥락에 맞는 이미지를 직관적으로 선택 및 교체할 수 있습니다.
*   **수집량 정밀 제어**: 에디터 내 슬라이더 UI를 통해 검색어당 수집할 아티클 개수(1~10개)를 실시간으로 조절하여 생성 속도와 정보 밀도를 최적화할 수 있습니다.
